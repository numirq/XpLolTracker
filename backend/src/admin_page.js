...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:14px/1.45 "Segoe UI",system-ui,sans-serif}button,input,select{font:inherit}button{border:0;cursor:pointer}
    ...
    .grid{display:grid;grid-template-columns:1fr;gap:18px}.section-head{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-bottom:14px}.section-head h2{margin:0;font-size:18px}.badge{border-radius:999px;background:var(--red);padding:3px 9px;font-weight:700;font-size:12px}
    .toolbar{display:grid;grid-template-columns:minmax(220px,1fr) 190px;gap:10px;margin:14px 0}.friend{background:var(--alt);border:1px solid var(--line);padding:16px;margin-top:10px}.friend.off{opacity:.68}.friend-top{display:flex;justify-content:space-between;align-items:flex-start;gap:12px}.friend h3{margin:0}.hint{color:var(--muted);font:12px Consolas,monospace}.chips{display:flex;flex-wrap:wrap;gap:6px;margin-top:8px}.chip{display:inline-flex;align-items:center;background:#0c1424;border:1px solid var(--line);color:var(--muted);padding:3px 8px;border-radius:999px;font-size:11px}.chip.alert{border-color:var(--red);color:var(--red)}.actions{display:flex;flex-wrap:wrap;gap:7px;justify-content:flex-end}.account{display:flex;justify-content:space-between;align-items:center;background:#0c1424;padding:9px 11px;margin-top:8px;border-left:3px solid var(--teal)}.account span{overflow-wrap:anywhere}
    .friend-devices{margin-top:14px;padding-top:12px;border-top:1px solid var(--line)}.friend-devices h4{margin:0 0 7px;font-size:13px;color:var(--muted)}.device{border:1px solid var(--line);padding:12px;margin-top:9px;background:#0c1424}.device.new{border-color:var(--red);box-shadow:inset 3px 0 var(--red)}.device-title{display:flex;justify-content:space-between;gap:8px}.meta{color:var(--muted);font-size:12px;margin-top:4px}.danger-text{color:var(--red);font-weight:700}.ok-text{color:var(--green)}
    ...
    @media(max-width:980px){.grid{grid-template-columns:1fr}.stats{grid-template-columns:repeat(2,1fr)}.shell{padding:16px}}@media(max-width:600px){.top,.friend-top,.row{align-items:stretch;flex-direction:column}.toolbar{grid-template-columns:1fr}.stats{grid-template-columns:1fr 1fr}.actions{justify-content:flex-start}.row .fit{flex:auto}}
  </style>
</head>
<body>
  <main class="shell">
    ...
    <section id="panel" class="hidden">
      ...
      <div class="grid">
        <section class="card">
          ...
          <div class="toolbar">
            <input id="friend-filter" type="search" placeholder="Szukaj znajomego, Riot ID lub urządzenia">
            <select id="friend-status"><option value="all">Wszystkie profile</option><option value="active">Tylko aktywne</option><option value="disabled">Tylko wyłączone</option><option value="alerts">Tylko z alertami</option></select>
          </div>
          <div id="friends"></div>
        </section>
      </div>
  ...
  (()=>{
    ...
    function render(){
      ...
      $('alert-card').classList.toggle('alert',data.alerts>0);$('retention').textContent='logi z '+data.retention_days+' dni';renderFriends();renderActivity();
    }
    function renderFriends(){const root=$('friends');root.replaceChildren();if(!data.friends.length){root.append(el('div','empty','Nie dodano jeszcze znajomych.'));return}const query=$('friend-filter').value.trim().toLocaleLowerCase('pl');const status=$('friend-status').value;let shown=0;
      data.friends.forEach(friend=>{const accounts=data.accounts.filter(a=>a.friend_id===friend.id);const devices=data.devices.filter(device=>device.friend_id===friend.id);const alerts=devices.filter(device=>!device.trusted).length;const searchable=[friend.name,...accounts.map(a=>a.game_name+'#'+a.tag_line),...devices.map(d=>d.name||d.id)].join(' ').toLocaleLowerCase('pl');if(query&&!searchable.includes(query))return;if(status==='active'&&!friend.enabled)return;if(status==='disabled'&&friend.enabled)return;if(status==='alerts'&&!alerts)return;shown+=1;const card=el('article','friend'+(friend.enabled?'':' off'));const top=el('div','friend-top');const title=el('div');title.append(el('h3','',friend.name),el('div','hint','kod: ••••••'+friend.token_hint+' • '+(friend.enabled?'aktywny':'wyłączony')));const chips=el('div','chips');chips.append(el('span','chip',accounts.length+' kont Riot'),el('span','chip'+(alerts?' alert':''),devices.length+' urządzeń'+(alerts?' • '+alerts+' nowych':'')));title.append(chips);const actions=el('div','actions');
        actions.append(button('Zmień nazwę','secondary',()=>renameFriend(friend)),button('Dodaj konto','secondary',()=>addAccount(friend)),button('Zmień kod','secondary',()=>rotate(friend)),button(friend.enabled?'Wyłącz kod':'Włącz kod',friend.enabled?'danger':'',()=>toggleFriend(friend)),button('Usuń znajomego','danger',()=>removeFriend(friend)));top.append(title,actions);card.append(top);
        if(!accounts.length)card.append(el('div','meta','Pierwsze konto przypisze się automatycznie, gdy znajomy użyje zaproszenia.'));accounts.forEach(account=>{const row=el('div','account');row.append(el('span','',account.game_name+'#'+account.tag_line+' • '+account.platform),button('Usuń','danger',()=>removeAccount(friend,account)));card.append(row)});
        const deviceSection=el('div','friend-devices');const heading=el('h4','',devices.length?'Urządzenia tego znajomego ('+devices.length+')':'Urządzenia tego znajomego');deviceSection.append(heading);if(!devices.length)deviceSection.append(el('div','meta','Brak zarejestrowanych urządzeń.'));devices.forEach(device=>deviceSection.append(renderDevice(device)));card.append(deviceSection);root.append(card)});if(!shown)root.append(el('div','empty','Brak profili pasujących do filtrów.'));
    }
    function renderDevice(device){const card=el('article','device'+(device.trusted?'':' new'));const title=el('div','device-title');title.append(el('strong',device.trusted?'ok-text':'danger-text',device.name||('Urządzenie '+device.id.slice(0,8))));if(!device.trusted)title.append(el('span','danger-text','NOWE'));const latest=data.activity.find(item=>item.device_id===device.id&&item.requested_account);card.append(title,el('div','meta','Ostatnie konto: '+(latest?.requested_account||'—')+' • ostatnio '+fmt(device.last_seen_at)+' • kraj '+(device.last_country||'—')+' • aplikacja '+(device.app_version||'starsza wersja')));const actions=el('div','actions');actions.style.marginTop='9px';if(!device.trusted)actions.append(button('To znane urządzenie','',()=>trustDevice(device)));actions.append(button('Nazwij','secondary',()=>nameDevice(device)),button('Usuń','danger',()=>removeDevice(device)));card.append(actions);return card}
    function renderActivity(){const root=$('activity');root.replaceChildren();if(!data.activity.length){const row=el('tr');const cell=el('td','empty','Brak aktywności.');cell.colSpan=6;row.append(cell);root.append(row);return}const labels={new_device:'Nowe urządzenie',first_account_claimed:'Przypisano pierwsze konto',request_ok:'Pobrano mecz',denied_account:'Odmowa dla konta',code_rotated:'Zmieniono kod'};data.activity.forEach(item=>{const row=el('tr');[fmt(item.occurred_at),item.friend_name||'—',labels[item.event_type]||item.event_type,item.requested_account||'—',(item.device_name||item.device_id?.slice(0,8)||'—')+' / '+(item.country||'—'),item.result].forEach(value=>row.append(el('td','',value)));root.append(row)})}
