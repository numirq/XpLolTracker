  ...
  (()=>{
    ...
    function renderFriends(){const root=$('friends');root.replaceChildren();if(!data.friends.length){root.append(el('div','empty','Nie dodano jeszcze znajomych.'));return}
      data.friends.forEach(friend=>{const card=el('article','friend'+(friend.enabled?'':' off'));const top=el('div','friend-top');const title=el('div');title.append(el('h3','',friend.name),el('div','hint','kod: ••••••'+friend.token_hint+' • '+(friend.enabled?'aktywny':'wyłączony')));const actions=el('div','actions');
        actions.append(button('Dodaj konto','secondary',()=>addAccount(friend)),button('Zmień kod','secondary',()=>rotate(friend)),button(friend.enabled?'Wyłącz kod':'Włącz kod',friend.enabled?'danger':'',()=>toggleFriend(friend)),button('Usuń znajomego','danger',()=>removeFriend(friend)));top.append(title,actions);card.append(top);
        const accounts=data.accounts.filter(a=>a.friend_id===friend.id);if(!accounts.length)card.append(el('div','meta','Pierwsze konto przypisze się automatycznie, gdy znajomy użyje zaproszenia.'));accounts.forEach(account=>{const row=el('div','account');row.append(el('span','',account.game_name+'#'+account.tag_line+' • '+account.platform),button('Usuń','danger',()=>removeAccount(friend,account)));card.append(row)});
        const devices=data.devices.filter(device=>device.friend_id===friend.id);const deviceSection=el('div','friend-devices');const heading=el('h4','',devices.length?'Urządzenia tego znajomego ('+devices.length+')':'Urządzenia tego znajomego');deviceSection.append(heading);if(!devices.length)deviceSection.append(el('div','meta','Brak zarejestrowanych urządzeń.'));devices.forEach(device=>deviceSection.append(renderDevice(device)));card.append(deviceSection);root.append(card)});
    }
